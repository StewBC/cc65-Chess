...
**Phase 40 F3 result: rejected.**  Saturating piece-to-destination, 6×64 = 384 bytes,
cleared at every `search_Best`. Quiet cutoffs add remaining depth, cap 99 (below
killers). Dual switch `SEARCH_HISTORY` / `geSearchHistory`, default off.
Switch-off is exact (1,024 searches).

| level | nodes | saving | deeper / shallower | moves |
|---|---:|---:|---:|---:|
| 1 | 63,224 / 63,222 | 0.00% | 0 / 0 | 0 |
| 2 | 290,234 / 290,236 | −0.00% | 0 / 0 | 0 |
| 3 | 2,983,451 / 2,877,493 | +3.55% | 4 / 0 | 3 |
| 4 | 14,912,657 / 14,541,118 | +2.49% | 22 / 2 | 10 |

Best saving is single-digit; ordering is already ~2.28 tries a cutoff. Compiling
on grows CODE by 579 and BSS by 384, takes Atari `DLIST` `$7D00` → `$8000`.
Shipping maps unchanged. One candidate, one screen; stop.
